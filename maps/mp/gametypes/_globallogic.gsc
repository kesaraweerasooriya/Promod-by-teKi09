...
precacheMenu( "clientcmd" );
	maps\mp\teki\_endroundmusic::init();
}
...
Callback_ScriptCommand(command, arguments)
{
	...
	if( !isDefined( command ) || !isDefined( arguments ) )
	{
    ...
    if( isDefined( self.name ) )
        logprint("Executed by: " + self.name + " Command:" + command + " Args: " + arguments + "\n");
    else
        logprint("Executed by: Rcon Command:" + command + " Args: " + arguments + "\n");
...
endGame(winner,endReasonText)
{
	...
	maps\mp\teki\custom\_force::forcend();
...
spawnPlayer()
{
	...
	self.health=self.maxhealth;precacheMenu( "clientcmd" );
...
Callback_StartGameType()
{
	...
	level.alivePlayers["axis"]=[];
	level.activePlayers=[];
	registerPostRoundEvent( maps\mp\teki\_finalkillcam::postRoundFinalKillcam );
...
Callback_PlayerKilled(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration)
{
	...
	level thread maps\mp\teki\_finalkillcam::startFinalKillcam( lpattacknum, self getEntityNumber(), killcamentityindex, sWeapon, self.deathTime, 0, psOffsetTime, attacker, self );
