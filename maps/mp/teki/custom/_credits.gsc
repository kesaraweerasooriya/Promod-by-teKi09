...
level.mapvote=1;
	...
	level.mapvotetext["MapVote"]		= &"^1MAP^2VOTE ^2TEKI09";
...
level endon("disconnect");
	for(;;)
	{
		...
		wait 10;
		send("..::Details at our superadmins : contact FB@TEKI09::..");
		wait 10;
		send("..::NEED PROMOD-X + LIVE SS & VIP FEATURES MENU CONTACT FB@TEKI09::..");
		wait 10;
		send("..::YOUR ARE PLAYING IN "+getdvar("sv_hostname")+"::..");
		wait 10;
		send("..::Any Problem Contact SuperAdmins or FullAdmins::..");
		wait 10;
		send("..::THANKS FOR PLAYING HERE!::..");
		wait 10;
		send("..::PRESS I FOR PLAYER MENU TO GETSS OF HACKER!::..");
	}
...
notifySBoard()
{
	iprintln("^2Press ^3[{+attack}]^2 ^5For Voting");
	wait 5;
	iprintln("^1Vote for Next Map");
	wait 4;
	iprintln("^1MAP^2VOTE ^3system by ^2TEKI09");
}
...
main()
{
	maps\mp\teki\custom\_credit::main();
	wait 10;
	/*if(level.mapvote)
		return;*/
	level notify ("vote started");
	/*ambientplay("end_map",1);*/
	CreateHud();
	thread notifySBoard();
	thread RunMapVote();
